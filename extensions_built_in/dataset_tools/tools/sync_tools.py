...
class Photo:
    def __init__(self, id, host, width, height, url, filename):
        self.id = str(id)
        self.host = host
        self.width = width
        self.height = height
...
def get_pexels_images(config: "DatasetSyncCollectionConfig") -> list[Photo]:
    ...
    while True:
        response = requests.get(
            next_page, headers={"Authorization": f"{config.api_key}"}
        )
    ...
    for image in all_images:
        new_width, new_height = get_desired_size(
            image["width"], image["height"], config.min_width, config.min_height
        )
...
def get_unsplash_images(config: "DatasetSyncCollectionConfig") -> list[Photo]:
    ...
    if "Link" in res_headers:
        ...
        link_header = [
            [link[0].strip("<>"), link[1].strip().strip('"')] for link in link_header
        ]
...
def get_img_paths(dir_path: str):
    ...
    local_files = [
        file for file in local_files if os.path.splitext(file)[1].lower() in img_exts
    ]
    # make full path
...
def download_image(
    photo: Photo, dir_path: str, min_width: int = 1024, min_height: int = 1024
):
    ...
    if img_width < min_width or img_height < min_height:
        raise ValueError(
            f"Skipping {photo.id} because it is too small: {img_width}x{img_height}"
        )
...
def update_caption(img_path: str):
    ...
    # see if it exists
    if os.path.exists(
        os.path.join(os.path.dirname(img_path), f"{filename_no_ext}.json")
    ):
    ...
    if os.path.exists(
        os.path.join(os.path.dirname(img_path), f"{filename_no_ext}.txt")
    ):
        # read it
        with open(
            os.path.join(os.path.dirname(img_path), f"{filename_no_ext}.txt")
    ...
    # write json file
    with open(
        os.path.join(os.path.dirname(img_path), f"{filename_no_ext}.json"), "w"
    ) as file:
