...
class DatasetSyncCollectionConfig:
    def __init__(self, **kwargs):
        ...
        self.collection_id: str = kwargs.get("collection_id")
...
class ImageState:
    ...
    def to_dict(self):
        return {"steps_complete": self.steps_complete}
...
class ImgInfo:
    def __init__(self, **kwargs):
        ...
        self.caption_short: str = kwargs.get("caption_short")
        self.poi = [Rect(**poi) for poi in kwargs.get("poi", [])]
        self.state = ImageState(**kwargs.get("state", {}))
        self.caption_method = kwargs.get("caption_method")
    ...
    def _upgrade_state(self):
        ...
        if self.caption is not None and "caption" not in self.state.steps_complete:
            ...
            self.is_dirty = True
        if (
            self.caption_short is not None
            and "caption_short" not in self.state.steps_complete
        ):
    ...
    def add_step(self, step: Step):
        if (
            step not in self.state.steps_to_complete
            and step not in self.state.steps_complete
        ):
    ...
    def add_steps(self, steps: list[Step]):
        ...
        is_manipulating_image = any(
            [step in img_manipulation_steps for step in self.state.steps_to_complete]
        )
        ...
        if not is_manipulating_image:
            ...
            target_img_manipulation_order = [
                step for step in steps if step in img_manipulation_steps
            ]
            current_img_manipulation_order = [
                step
    ...
    def set_caption_method(self, method: str):
        ...
        if self.caption_method != method:
            ...
            if (
                self.caption_method is not None
                and self.caption is not None
                or self.caption_short is not None
            ):
            ...
            if method in self.other_captions:
                ...
                self.caption_short = self.other_captions[method].get(
                    "caption_short", None
                )
            else:
