...
from toolkit.config_modules import ReferenceDatasetConfig, SliderConfig
...
from toolkit.prompt_utils import (
    ...
    EncodedPromptPair,
    PromptEmbedsCache,
    build_latent_image_batch_for_prompt_pair,
...
class UltimateSliderTrainerProcess(BaseSDTrainProcess):
    ...
    def __init__(self, process_id: int, job, config: OrderedDict, **kwargs):
        ...
        self.train_with_dataset = (
            self.slider_config.datasets is not None
            and len(self.slider_config.datasets) > 0
        )

    def load_datasets(self):
        if (
            self.data_loader is None
            and self.slider_config.datasets is not None
            and len(self.slider_config.datasets) > 0
        ):
    ...
    def hook_train_loop(self, batch):
        ...
        with torch.no_grad():
            ...
            positive_latents_chunks = torch.chunk(
                positive_latents, self.prompt_chunk_size, dim=0
            )
            neutral_latents_chunks = torch.chunk(
                neutral_latents, self.prompt_chunk_size, dim=0
            )
            unconditional_latents_chunks = torch.chunk(
                unconditional_latents, self.prompt_chunk_size, dim=0
            )
            prompt_pair_chunks = split_prompt_pairs(prompt_pair, self.prompt_chunk_size)
            noisy_cfg_latents_chunks = torch.chunk(
                noisy_cfg_latents, self.prompt_chunk_size, dim=0
            )
            ...
            for (
                ...
                if (
                    ...
                    timesteps_index_list = [
                        current_timestep_index for _ in range(target_latents.shape[0])
                    ]
                    # add min_snr_gamma
                    loss = apply_snr_weight(
                        loss,
                ...
                loss = (
                    ...
                    * self.slider_config.cfg_loss_weight
                )
        ...
        reference_image_loss = (
            sum(reference_image_losses) / len(reference_image_losses)
            if len(reference_image_losses) > 0
            else 0.0
        )
        cfg_loss = (
            sum(cfg_loss_list) / len(cfg_loss_list) if len(cfg_loss_list) > 0 else 0.0
        )
