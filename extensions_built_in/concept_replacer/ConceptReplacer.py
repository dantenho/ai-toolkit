...
class ConceptReplacer(BaseSDTrainProcess):
    def __init__(self, process_id: int, job, config: OrderedDict, **kwargs):
        ...
        self.replacement_list = [
            ConceptReplacementConfig(**x) for x in replacement_list
        ]
    ...
    def hook_train_loop(self, batch):
        with torch.no_grad():
            ...
            for idx, replacement in enumerate(batch_replacement_list):
                ...
                concept_prompts.append(
                    f"{shuffled_concept}, {prompt}, {shuffled_concept}"
                )

                # insert replacement at beginning and end of prompt
                shuffled_replacement = [
                    x.strip() for x in replacement.replacement.split(",")
                ]
                random.shuffle(shuffled_replacement)
                shuffled_replacement = ", ".join(shuffled_replacement)
                replacement_prompts.append(
                    f"{shuffled_replacement}, {prompt}, {shuffled_replacement}"
                )

            # predict the replacement without network
            conditional_embeds = self.sd.encode_prompt(replacement_prompts).to(
        ...
        with network:
            with torch.set_grad_enabled(grad_on_text_encoder):
                # embed the prompts
                conditional_embeds = self.sd.encode_prompt(concept_prompts).to(
                    self.device_torch, dtype=dtype
                )
            ...
            loss = torch.nn.functional.mse_loss(
                noise_pred.float(), replacement_pred.float(), reduction="none"
            )
