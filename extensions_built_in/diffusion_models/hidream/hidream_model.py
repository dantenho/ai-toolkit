...
from .src.models.transformers.transformer_hidream_image import (
...
scheduler_config = {"num_train_timesteps": 1000, "shift": 3.0}
...
class HidreamModel(BaseModel):
    ...
    def load_model(self):
        ...
        llama_model_path = self.model_config.model_kwargs.get(
        ...
        if self.model_config.quantize:
            ...
            if self.low_vram:
                ...
                all_blocks = list(transformer.double_stream_blocks) + list(
        ...
        text_encoder = CLIPTextModelWithProjection.from_pretrained(
            extras_path, subfolder="text_encoder", torch_dtype=torch.bfloat16
        ).to(self.device_torch, dtype=dtype)

        tokenizer = CLIPTokenizer.from_pretrained(extras_path, subfolder="tokenizer")

        text_encoder_2 = CLIPTextModelWithProjection.from_pretrained(
            extras_path, subfolder="text_encoder_2", torch_dtype=torch.bfloat16
        ).to(self.device_torch, dtype=dtype)
        ...
        text_encoder_3 = T5EncoderModel.from_pretrained(
            extras_path, subfolder="text_encoder_3", torch_dtype=torch.bfloat16
        ).to(self.device_torch, dtype=dtype)
        ...
        tokenizer_3 = T5Tokenizer.from_pretrained(extras_path, subfolder="tokenizer_3")
        flush()
