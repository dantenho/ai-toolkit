from .chroma import ChromaModel, ChromaRadianceModel
from .f_light import FLiteModel
from .flux2 import Flux2Model
from .flux_kontext import FluxKontextModel
from .hidream import HidreamE1Model, HidreamModel
from .omnigen2 import OmniGen2Model
from .qwen_image import QwenImageEditModel, QwenImageEditPlusModel, QwenImageModel
from .wan22 import Wan225bModel, Wan2214bI2VModel, Wan2214bModel
from .z_image import ZImageModel

AI_TOOLKIT_MODELS = [
    # put a list of models here
    ChromaModel,
    ChromaRadianceModel,
    HidreamModel,
    HidreamE1Model,
    FLiteModel,
    OmniGen2Model,
    FluxKontextModel,
    Wan225bModel,
    Wan2214bI2VModel,
    Wan2214bModel,
    QwenImageModel,
    QwenImageEditModel,
    QwenImageEditPlusModel,
    Flux2Model,
    ZImageModel,
]
