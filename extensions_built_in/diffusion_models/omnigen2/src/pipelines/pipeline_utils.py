...
def get_pipeline_embeds(pipeline, prompt, negative_prompt, device):
    ...
    input_ids = pipeline.tokenizer(
        prompt, return_tensors="pt", truncation=False, padding="longest"
    ).input_ids.to(device)
    ...
    for i in range(0, shape_max_length, max_length):
        ...
        concat_embeds.append(
            pipeline.text_encoder(
                input_ids[:, i : i + max_length], attention_mask=attention_mask
            )[0]
        )
        ...
        if negative_prompt is not None:
            if (
                ...
                attention_mask = negative_ids[:, i : i + max_length].attention_mask.to(
                    device
                )
            else:
                attention_mask = None
            neg_embeds.append(
                pipeline.text_encoder(
                    negative_ids[:, i : i + max_length], attention_mask=attention_mask
                )[0]
            )
