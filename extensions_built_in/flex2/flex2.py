...
class Flex2(BaseModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.invert_inpaint_mask_chance = model_config.model_kwargs.get(
        ...
        self.inpaint_random_chance = model_config.model_kwargs.get(
            "inpaint_random_chance", 0.0
        )
        self.random_blur_mask = model_config.model_kwargs.get("random_blur_mask", False)
        self.random_dialate_mask = model_config.model_kwargs.get(
            "random_dialate_mask", False
        )
        self.do_random_inpainting = model_config.model_kwargs.get(
    ...
    def condition_noisy_latents(
        ...
        with torch.no_grad():
            ...
            do_dropout = (
                random.random() < self.inpaint_dropout
                if self.inpaint_dropout > 0.0
                else False
            )
            ...
            if inpaint_tensor is not None and not do_dropout:
                ...
                inpainting_tensor_mask = F.interpolate(
                    ...
                    size=(inpainting_latent.shape[2], inpainting_latent.shape[3]),
                    mode="bilinear",
                )
                ...
                if self.random_dialate_mask:
                    inpainting_tensor_mask = random_dialate_mask(
                        inpainting_tensor_mask, max_percent=0.05
                    )
            ...
            do_dropout = (
                random.random() < self.control_dropout
                if self.control_dropout > 0.0
                else False
            )
            if do_dropout:
                # dropout with zeros
