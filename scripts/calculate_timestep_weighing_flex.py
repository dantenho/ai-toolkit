...
with torch.no_grad():
    transformer = FluxTransformer2DModel.from_pretrained(
        base_model_path, subfolder="transformer", torch_dtype=dtype
    )
    ...
    prompt_embeds, pooled_prompt_embeds, text_ids = pipe.encode_prompt(
        prompt, prompt_2=prompt, device=device
    )
    ...
    packed_latents = pipe._pack_latents(
        latents, 1, num_channels_latents, l_height, l_width
    )
    ...
    timestep_weights = torch.zeros(
        num_train_timesteps, dtype=torch.float32, device=device
    )
