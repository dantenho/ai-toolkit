                ...
                if attributes.get("patron_status") == "active_patron":
                    ...
                    if user_id:
                        for included in members_data.get("included", []):
                            if (
                                included.get("id") == user_id
                                and included.get("type") == "user"
                            ):
                                profile_image = included.get("attributes", {}).get(
                                    "image_url"
                                )
                                break
...
def fetch_github_sponsors():
    ...
    try:
        ...
        print(
            f"Found {len(sponsors)} active GitHub sponsors for {entity_type} '{entity_name}'"
        )
        return sponsors
        ...
        if github_sponsors:
            ...
            for sponsor in github_sponsors:
                if sponsor["profile_image"]:
                    f.write(
                        f'<a href="{sponsor["profile_url"]}" title="{sponsor["name"]}"><img src="{sponsor["profile_image"]}" width="50" height="50" alt="{sponsor["name"]}" style="border-radius:50%;display:inline-block;"></a> '
                    )
                else:
        ...
        if patreon_supporters:
            ...
            for supporter in patreon_supporters:
                if supporter["profile_image"]:
                    f.write(
                        f'<a href="{supporter["profile_url"]}" title="{supporter["name"]}"><img src="{supporter["profile_image"]}" width="50" height="50" alt="{supporter["name"]}" style="border-radius:50%;display:inline-block;"></a> '
                    )
                else:
