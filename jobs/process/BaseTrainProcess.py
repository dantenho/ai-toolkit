...
class BaseTrainProcess(BaseProcess):
    def __init__(self, process_id: int, job, config: OrderedDict):
        ...
        self.job: TrainJob | BaseJob | ExtensionJob
        self.progress_bar: tqdm = None

        self.training_seed = self.get_conf(
            "training_seed",
            self.job.training_seed if hasattr(self.job, "training_seed") else None,
        )
        ...
        self.writer = None
        self.training_folder = self.get_conf(
            "training_folder",
        ...
        self.step = 0
        self.first_step = 0
        self.log_dir = self.get_conf(
            "log_dir", self.job.log_dir if hasattr(self.job, "log_dir") else None
        )
