...
class GenerateConfig:
    def __init__(self, **kwargs):
        ...
        self.size_list: list[int] | None = kwargs.get("size_list")
        self.neg = kwargs.get("neg", "")
        self.seed = kwargs.get("seed", -1)
        self.guidance_scale = kwargs.get("guidance_scale", 7)
        self.sample_steps = kwargs.get("sample_steps", 20)
        self.prompt_2 = kwargs.get("prompt_2")
        self.neg_2 = kwargs.get("neg_2")
        ...
        if isinstance(self.prompts, str):
            if os.path.exists(self.prompts):
                with open(self.prompts, encoding="utf-8") as f:
                    ...
                    self.prompts_in_file = [
                        p.strip() for p in self.prompts_in_file if len(p.strip()) > 0
                    ]
            else:
                raise ValueError(
                    "Prompts file does not exist, put in list if you want to use a list of prompts"
                )
        ...
        if self.random_prompts:
            ...
            for i in range(self.max_images):
                ...
                prompt_list = [
                    random.choice(self.prompts_in_file) for _ in range(num_prompts)
                ]
    ...
    def run(self):
        with torch.no_grad():
            ...
            self.sd.generate_images(
                prompt_image_configs, sampler=self.generate_config.sampler
            )
