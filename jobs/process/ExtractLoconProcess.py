...
mode_dict = {
    "fixed": {"linear": 64, "conv": 32, "type": int},
    "threshold": {"linear": 0, "conv": 0, "type": float},
    "ratio": {"linear": 0.5, "conv": 0.5, "type": float},
    "quantile": {"linear": 0.5, "conv": 0.5, "type": float},
}
...
class ExtractLoconProcess(BaseExtractProcess):
    def __init__(self, process_id: int, job, config: OrderedDict):
        ...
        self.linear_param = self.get_conf(
            "linear",
            mode_dict[self.mode]["linear"],
            as_type=mode_dict[self.mode]["type"],
        )
        self.conv_param = self.get_conf(
            "conv", mode_dict[self.mode]["conv"], as_type=mode_dict[self.mode]["type"]
        )

    def run(self):
        super().run()
        print(
            f"Running process: {self.mode}, lin: {self.linear_param}, conv: {self.conv_param}"
        )
