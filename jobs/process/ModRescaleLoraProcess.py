...
class ModRescaleLoraProcess(BaseProcess):
    ...
    def __init__(self, process_id: int, job, config: OrderedDict):
        ...
        self.save_dtype = self.get_conf(
            "save_dtype", default="fp16", as_type=get_torch_dtype
        )
        self.current_weight = self.get_conf(
            "current_weight", required=True, as_type=float
        )
        self.target_weight = self.get_conf(
            "target_weight", required=True, as_type=float
        )
        self.scale_target = self.get_conf(
            "scale_target", default="up_down"
        )  # alpha or up_down
    ...
    def run(self):
        ...
        if self.replace_meta:
            ...
            save_meta = get_meta_for_safetensors(
                source_meta, self.job.name, add_software_info=False
            )
        ...
        for key in list(source_state_dict.keys()):
            ...
            up_down_scale = torch.pow(total_module_scale, 1.0 / num_modules_layers).to(
                "cpu", dtype=get_torch_dtype("fp32")
            )
            # only update alpha
            if self.scale_target == "alpha" and key.endswith(".alpha"):
                v = v * total_module_scale
            if (
                self.scale_target == "up_down"
                and key.endswith(".lora_up.weight")
