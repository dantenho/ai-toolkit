...
class ExtractJob(BaseJob):
    ...
    def run(self):
        ...
        # (text_model, vae, unet)
        self.model_base = load_models_from_stable_diffusion_checkpoint(
            self.is_v2, self.base_model_path
        )
        ...
        self.model_extract = load_models_from_stable_diffusion_checkpoint(
            self.is_v2, self.extract_model_path
        )
