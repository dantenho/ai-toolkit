...
KEYMAPS_FOLDER = os.path.join(
    os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "toolkit", "keymaps"
)
...
parser.add_argument(
    "--name", type=str, default="stable_diffusion", help="name for mapping to make"
)
...
if args.sd2:
    ...
    proj_pattern_weight = (
        r"cond_stage_model\.model\.transformer\.resblocks\.(\d+)\.attn\.in_proj_weight"
    )
    proj_pattern_bias = (
        r"cond_stage_model\.model\.transformer\.resblocks\.(\d+)\.attn\.in_proj_bias"
    )
...
if args.sdxl or args.sd2 or args.ssd or args.refiner or args.vega:
    if "conditioner.embedders.1.model.text_projection" in ldm_dict_keys:
        ...
        d_model = int(
            ldm_state_dict["conditioner.embedders.1.model.text_projection"].shape[0]
        )
        ...
        # d_model = int(checkpoint[prefix + "text_projection"].shape[0]))
        d_model = int(
            ldm_state_dict["conditioner.embedders.0.model.text_projection"].shape[0]
        )
    else:
        d_model = 1024
    ...
    for ldm_key in ldm_dict_keys:
        try:
            ...
            if match:
                ...
                diffusers_state_dict[
                    f"te{te_suffix}_text_model.encoder.layers.{number}.self_attn.{tmp_merge_key}.weight"
                ] = new_val
            ...
            if match:
                ...
                # make diffusers convertable_dict
                diffusers_state_dict[
                    f"te{te_suffix}_text_model.encoder.layers.{number}.self_attn.{tmp_merge_key}.bias"
                ] = new_val
    ...
    for diffusers_key in diffusers_dict_keys:
        ...
        if mse < error_margin:
            ...
            if did_reduce_ldm or did_reduce_diffusers:
                ldm_diffusers_shape_map[ldm_key] = (
                    ldm_shape_tuple,
                    diffusers_shape_tuple,
                )
                if did_reduce_ldm:
                    del ldm_weight
...
unmatched_diffusers_keys = [
    x for x in diffusers_dict_keys if x not in matched_diffusers_keys
]
# has unmatched keys
...
if has_unmatched_keys:
    ...
    print("Gathering info on unmatched keys")
...
save_file(
    remaining_ldm_values, os.path.join(KEYMAPS_FOLDER, f"{name}_ldm_base.safetensors")
)
print(
    f"Saved remaining ldm values to {os.path.join(KEYMAPS_FOLDER, f'{name}_ldm_base.safetensors')}"
)
