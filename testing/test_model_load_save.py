...
parser.add_argument("file_1", nargs="+", type=str, help="Path an LDM model")

parser.add_argument("--is_xl", action="store_true", help="Is the model an XL model")

parser.add_argument("--is_v2", action="store_true", help="Is the model a v2 model")
...
model_config = ModelConfig(name_or_path=args.file_1[0], is_xl=args.is_xl)
sd = StableDiffusion(
...
state_dict_file_2 = get_ldm_state_dict_from_diffusers(
    sd.state_dict(), version_string, device="cpu", dtype=dtype
)
...
with open(
    os.path.join(project_root, "config", f"{state_dict_1_filename}_loop.json"), "w"
) as f:
