...
def reduce_weight(weight, target_size):
    ...
    new_weight = torch.nn.functional.interpolate(
        tmp_weight,
        size=(target_size, weight.shape[1]),
        mode="bicubic",
        align_corners=True,
    )
...
def reduce_bias(bias, target_size):
    ...
    new_bias = torch.nn.functional.interpolate(
        bias, size=(target_size, 1), mode="bicubic", align_corners=True
    )
...
for key, value in state_dict.items():
    ...
    if "weight" in key or "scale_shift_table" in key:
        ...
        if (
            len(value.shape) > 1
            and value.shape[1] == 1152
            and "attn2.to_k.weight" not in key
            and "attn2.to_v.weight" not in key
        ):
            value = (
                reduce_weight(value.t(), 512).t().contiguous()
            )  # Transpose before and after reduction
            pass
        elif len(value.shape) > 1 and value.shape[1] == 4608:
            value = (
                reduce_weight(value.t(), 2048).t().contiguous()
            )  # Transpose before and after reduction
            pass
