...
class FrameEmbedder(torch.nn.Module):
    ...
    def from_model(
        ...
        if model.__class__.__name__ == "WanTransformer3DModel":
            ...
            orig_patch_embedding._orig_i2v_adapter_forward = (
                orig_patch_embedding.forward
            )
    ...
    def forward(self, x):
        ...
        x = x.to(
            self.patch_embedding.weight.device, dtype=self.patch_embedding.weight.dtype
        )
...
def deactivatable_forward(self: "Attention", *args, **kwargs):
...
class AttentionHog(torch.nn.Module):
    def __init__(
        ...
        self.add_k_proj = torch.nn.Linear(
            added_kv_proj_dim, attn_layer.inner_kv_dim, bias=attn_layer.added_proj_bias
        )
        ...
        if attn_layer.norm_q is not None:
            ...
            if qk_norm == "layer_norm":
                self.norm_added_q = torch.nn.LayerNorm(
                    attn_layer.norm_q.normalized_shape,
                    eps=eps,
                    elementwise_affine=attn_layer.norm_q.elementwise_affine,
                )
                self.norm_added_k = torch.nn.LayerNorm(
                    attn_layer.norm_k.normalized_shape,
                    eps=eps,
                    elementwise_affine=attn_layer.norm_k.elementwise_affine,
                )
        ...
        if adapter.adapter_ref().is_sampling:
            ...
            if self._do_unconditional:
                ...
                encoder_hidden_states_image = shuffle_tensor_along_axis(
                    adapter.adapter_ref().conditional_embeds, axis=1
                )
...
class I2VAdapter(torch.nn.Module):
    def __init__(
        ...
        image_processor: SiglipImageProcessor | CLIPImageProcessor,
        vision_encoder: SiglipVisionModel | CLIPVisionModelWithProjection,
    ):
        super().__init__()
        ...
        if self.config.i2v_do_start_frame:
            self.frame_embedder = FrameEmbedder.from_model(sd.unet, self)
        ...
        if model_class == "WanTransformer3DModel":
            added_kv_proj_dim = (
                sd.model.config.num_attention_heads * sd.model.config.attention_head_dim
            )
            ...
            for block in transformer.blocks:
                ...
                attn_module = AttentionHog(
                    added_kv_proj_dim, self, block.attn2, transformer
                )
        ...
        if model_class == "WanTransformer3DModel":
            ...
            self.sd_ref().model.forward = partial(new_wan_forward, self.sd_ref().model)
    ...
    def get_params(self):
        if self.control_lora is not None:
            ...
            params_net = self.control_lora.prepare_optimizer_params(**config)
    ...
    def load_weights(self, state_dict, strict=True):
        ...
        if self.frame_embedder is not None:
            self.frame_embedder.load_state_dict(frame_embedder_sd, strict=False)
        self.attn_hog_list.load_state_dict(attn_hog_sd, strict=False)
        self.image_embedder.load_state_dict(image_embedder_sd, strict=False)
