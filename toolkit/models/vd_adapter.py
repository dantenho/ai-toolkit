...
from toolkit.models.pixtral_vision import VisionLanguageAdapter
...
class MLPR(nn.Module):  # MLP with reshaping
    def __init__(self, in_dim, in_channels, out_dim, out_channels, use_residual=True):
        super().__init__()
        if use_residual:
            ...
            try:
                ...
                if adapter_hidden_states.shape[0] == batch_size // 2:
                    adapter_hidden_states = torch.cat(
                        [self.unconditional_embeds, adapter_hidden_states], dim=0
                    )
                ...
                vd_key = vd_key.view(batch_size, -1, attn.heads, head_dim).transpose(
                    1, 2
                )
                vd_value = vd_value.view(
                    batch_size, -1, attn.heads, head_dim
                ).transpose(1, 2)
                ...
                vd_hidden_states = vd_hidden_states.transpose(1, 2).reshape(
                    batch_size, -1, attn.heads * head_dim
                )
...
class CustomFluxVDAttnProcessor2_0(torch.nn.Module):
    ...
    def __call__(
        ...
        if self.is_active and self.conditional_embeds is not None:
            ...
            vd_value = vd_value.view(batch_size, -1, attn.heads, head_dim).transpose(
                1, 2
            )
    ...
    def __init__(
        ...
        for name in attn_processor_keys:
            if is_flux:
                ...
                cross_attention_dim = (
                    None
                    if name.endswith("attn1.processor") or name.endswith("attn.1")
                ...
                if to_k_adapter.shape[1] < self.mid_size:
                    to_k_adapter = torch.cat(
                        [
                            to_k_adapter,
                            torch.randn(
                                to_k_adapter.shape[0],
                                self.mid_size - to_k_adapter.shape[1],
                            ).to(to_k_adapter.device, dtype=to_k_adapter.dtype)
                            * 0.01,
                        ],
                        dim=1,
                    )
                    to_v_adapter = torch.cat(
                        [
                            to_v_adapter,
                            torch.randn(
                                to_v_adapter.shape[0],
                                self.mid_size - to_v_adapter.shape[1],
                            ).to(to_k_adapter.device, dtype=to_k_adapter.dtype)
                            * 0.01,
                        ],
                        dim=1,
        ...
        if self.sd_ref().is_pixart:
            ...
            self.adapter_modules = torch.nn.ModuleList(
                [
                    transformer.transformer_blocks[i].attn1.processor
        ...
        if self.config.train_scaler:
            self.block_scaler = torch.nn.Parameter(
                torch.tensor([0.0] * num_modules).to(
                    dtype=torch.float32, device=self.sd_ref().device_torch
                )
            )
        ...
        if self.config.conv_pooling:
            ...
            self.pool = nn.Sequential(
                nn.Conv1d(
                    sequence_length, self.config.conv_pooling_stacks, 1, bias=False
                ),
                Norm(),
            )
    ...
    def forward(self, input):
        ...
        if self.pool is not None:
            ...
            if self.config.conv_pooling_stacks > 1:
                input = torch.cat(
                    torch.chunk(input, self.config.conv_pooling_stacks, dim=1), dim=2
                )
