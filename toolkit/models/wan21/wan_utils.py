...
def add_first_frame_conditioning(latent_model_input, first_frame, vae):
    """
    ...
    video_condition = torch.cat(
        [first_frame, *[zero_frame for _ in range(num_frames - 1)]], dim=2
    )
...
def add_first_frame_conditioning_v22(
    latent_model_input, first_frame, vae, last_frame=None
):
    """
    ...
    first_frame_up = F.interpolate(
        first_frame, size=(target_h, target_w), mode="bilinear", align_corners=False
    )
    ...
    if last_frame is not None:
        ...
        last_frame_up = F.interpolate(
            last_frame, size=(target_h, target_w), mode="bilinear", align_corners=False
        )
        last_frame_up = last_frame_up.unsqueeze(2)
        last_encoded = (
            vae.encode(last_frame_up).latent_dist.sample().to(dtype).to(device)
        )
