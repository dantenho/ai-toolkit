...
class DiffusionFeatureExtractor3(nn.Module):
    def __init__(self, device=torch.device("cuda"), dtype=torch.bfloat16, vae=None):
        ...
        self.vision_encoder = SiglipVisionModel.from_pretrained(
            image_encoder_path, ignore_mismatched_sizes=True
    ...
    def get_siglip_features(self, tensors_0_1):
        ...
        images = F.interpolate(
            tensors_0_1, size=(size, size), mode="bicubic", align_corners=False
        )
        ...
        clip_image = (images - mean.view([1, 3, 1, 1])) / std.view([1, 3, 1, 1])
    ...
    def get_lpips_features(self, tensors_0_1):
        ...
        # do lpips
        lpips_feat_list = [
            x for x in get_lpips_features(tensors_n1p1.to(device, dtype=torch.float32))
        ]
...
class DiffusionFeatureExtractor4(nn.Module):
    ...
    def tensors_to_siglip_like_features(self, batch: torch.Tensor):
        ...
        patches = torch.nn.functional.unfold(
            batch, kernel_size=patch, stride=patch
        ).transpose(1, 2)
    ...
    def forward(
        ...
        if model is not None and hasattr(model, "get_stepped_pred"):
            ...
            stepped_latents = self.step_latents(
                noise, noise_pred, noisy_latents, timesteps, scheduler
            )
        ...
        if mse_weight > 0:
            mse_loss = (
                torch.nn.functional.mse_loss(pred_images.float(), target_img.float())
                * mse_weight
            )
...
class DiffusionFeatureExtractor5(DiffusionFeatureExtractor4):
    ...
    def step_latents(
        ...
        for idx in range(bs):
            model_output = noise_pred_chunks[
                idx
            ]  # predicted noise (same shape as latent)
