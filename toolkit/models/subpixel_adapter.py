    ...
    def from_model(
        ...
        if model.__class__.__name__ == "FluxTransformer2DModel":
            ...
            if x_embedder.weight.shape == in_out_module.x_embedder.weight.shape:
                in_out_module.x_embedder.weight.data = (
                    x_embedder.weight.data.clone().float()
                )
                in_out_module.x_embedder.bias.data = (
                    x_embedder.bias.data.clone().float()
                )
                in_out_module.proj_out.weight.data = (
                    proj_out.weight.data.clone().float()
                )
            ...
            sd.vae = AutoencoderPixelMixer(
                in_channels=3, downscale_factor=downscale_factor
            )
...
class SubpixelAdapter(torch.nn.Module):
    ...
    def load_weights(self, state_dict, strict=True):
        ...
        for key, value in state_dict.items():
            if "transformer.x_embedder" in key or "transformer.proj_out" in key:
