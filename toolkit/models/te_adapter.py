...
class TEAdapterCaptionProjection(nn.Module):
    ...
    def forward(self, caption):
        if self.is_active and self.conditional_embeds is not None:
            ...
            if (
                self.unconditional_embeds is not None
                and adapter_hidden_states.shape[0] != caption.shape[0]
            ):
                # concat unconditional to match the hidden state batch size
                if (
                    self.unconditional_embeds.text_embeds.shape[0] == 1
                    and adapter_hidden_states.shape[0] != 1
                ):
                    unconditional = torch.cat(
                        [self.unconditional_embeds.text_embeds]
                        * adapter_hidden_states.shape[0],
                        dim=0,
                    )
                else:
                    unconditional = self.unconditional_embeds.text_embeds
                adapter_hidden_states = torch.cat(
                    [unconditional, adapter_hidden_states], dim=0
                )
...
class TEAdapterAttnProcessor(nn.Module):
    ...
    def __call__(
        ...
        if self.is_active and self.conditional_embeds is not None:
            ...
            if (
                self.unconditional_embeds is not None
                and adapter_hidden_states.shape[0] != encoder_hidden_states.shape[0]
            ):
                # concat unconditional to match the hidden state batch size
                if (
                    self.unconditional_embeds.text_embeds.shape[0] == 1
...
class TEAdapter(torch.nn.Module):
    def __init__(
        ...
        if (
            self.adapter_ref().config.text_encoder_arch == "t5"
            or self.adapter_ref().config.text_encoder_arch == "pile-t5"
        ):
        ...
        if sd.is_xl:
            ...
            self.text_projection = nn.Linear(
                te.config.hidden_size,
                clip_with_projection.config.projection_dim,
                bias=False,
            )
                    ...
                    to_k_adapter = torch.cat(
                        [
                            ...
                            torch.randn(
                                ...
                                self.token_size - to_k_adapter.shape[1],
                    ...
                    to_v_adapter = torch.cat(
                        [
                            ...
                            torch.randn(
                                ...
                                self.token_size - to_v_adapter.shape[1],
                            ).to(to_k_adapter.device, dtype=to_k_adapter.dtype)
                            * 0.01,
                        ],
                        dim=1,
