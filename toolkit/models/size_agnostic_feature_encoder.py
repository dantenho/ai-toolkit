...
class SizeAgnosticFeatureEncoder(nn.Module):
    def __init__(
        ...
        self.unshuffle = nn.PixelUnshuffle(
            downscale_factor
        )  # (bs, 3, 256, 256) -> (bs, 192, 32, 32)

        self.conv_in = nn.Conv2d(
            subpixel_channels, reducer_channels, kernel_size=3, padding=1
        )  # (bs, 192, 32, 32) -> (bs, 512, 32, 32)

        # run as many times as needed to get to min feature of 8 on the smallest dimension
        self.reducer = SAFEReducerBlock(
            reducer_channels
        )  # (bs, 512, 32, 32) -> (bs, 512, 8, 8)
        ...
        # (bs, 2048, 8, 8)
        self.block1 = SAFEReducerBlock(
            self.channels
        )  # (bs, 2048, 8, 8) -> (bs, 2048, 4, 4)
        self.block2 = SAFEReducerBlock(
            self.channels
        )  # (bs, 2048, 8, 8) -> (bs, 2048, 2, 2)
...
class SAFEImageProcessor(BaseImageProcessor):
    def __init__(self, max_size=1024, min_size=256, **kwargs):
    ...
    def __call__(self, images, **kwargs):
        ...
        if images.min() < -0.3 or images.max() > 1.3:
            raise ValueError(
                f"images fed into SAFEImageProcessor values must be between 0 and 1. Got min: {images.min()}, max: {images.max()}"
            )
