...
class LLMAdapter(torch.nn.Module):
    def __init__(
        ...
        if sd.is_flux:
            ...
            self.context_embedder = nn.Linear(self.hidden_size, sd.unet.inner_dim)
            ...
            sd.unet.context_embedder._context_embedder_ref = weakref.ref(
            ...
            for idx in range(self.num_cloned_blocks):
                ...
                orig_block.forward = partial(new_block_forward, orig_block)
                orig_block._new_block_ref = weakref.ref(block)
                orig_block._adapter_ref = self.adapter_ref

        elif sd.is_lumina2:
            self.context_embedder = nn.Linear(self.hidden_size, sd.unet.hidden_size)
            self.sequence_length = 256
        else:
            raise ValueError("llm adapter currently only supports flux or lumina2")
