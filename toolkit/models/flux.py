...
def split_gpu_single_block_forward(
    ...
    **kwargs,
    ...
    hidden_state_out = self._pre_gpu_split_forward(
        hidden_states, temb, image_rotary_emb, joint_attention_kwargs, **kwargs
    )
...
def add_model_gpu_splitter_to_flux(
    ...
    other_module_params: int | None = 5e9,
    # since they are not trainable, multiply by smaller number
    other_module_param_count_scale: float | None = 0.3,
):
    ...
    total_params = (
        sum(p.numel() for p in transformer.parameters()) + other_module_params
    )
    ...
    transformer.single_transformer_blocks[-1]._split_output_device = torch.device(
        "cuda:0"
    )
