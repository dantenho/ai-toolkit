...
def convert_state_dict_to_ldm_with_mapping(
    ...
    if base_path is not None:
        ...
        for key in converted_state_dict:
            converted_state_dict[key] = converted_state_dict[key].to(
                device, dtype=dtype
            )
    ...
    for ldm_key in ldm_diffusers_operator_map:
        ...
        if "cat" in ldm_diffusers_operator_map[ldm_key]:
            ...
            converted_state_dict[ldm_key] = torch.cat(cat_list, dim=0).to(
                device, dtype=dtype
            )
        ...
        if "slice" in ldm_diffusers_operator_map[ldm_key]:
            tensor_to_slice = diffusers_state_dict[
                ldm_diffusers_operator_map[ldm_key]["slice"][0]
            ]
            slice_text = diffusers_state_dict[
                ldm_diffusers_operator_map[ldm_key]["slice"][1]
            ]
            converted_state_dict[ldm_key] = (
                tensor_to_slice[get_slices_from_string(slice_text)]
                .detach()
    ...
    for ldm_key in ldm_diffusers_keymap:
        ...
        if ldm_diffusers_keymap[ldm_key] in diffusers_state_dict:
            tensor = (
                diffusers_state_dict[ldm_diffusers_keymap[ldm_key]]
                .detach()
                .to(device, dtype=dtype)
            )
    ...
    missing_diffusers_keys = [
        x for x in mapped_diffusers_keys if x not in diffusers_matched_keys
    ]
...
def get_ldm_state_dict_from_diffusers(
    ...
    return convert_state_dict_to_ldm_with_mapping(
        state_dict, mapping_path, base_path, device=device, dtype=dtype
    )
...
def save_ldm_model_from_diffusers(
    ...
    converted_state_dict = get_ldm_state_dict_from_diffusers(
        sd.state_dict(), sd_version, device="cpu", dtype=save_dtype
    )
...
def load_t2i_model(
    path_to_file, device: str = "cpu", dtype: torch.dtype = torch.float32
):
...
def save_ip_adapter_from_diffusers(
    ...
    for module_name, state_dict in combined_state_dict.items():
        if direct_save:
            converted_state_dict[module_name] = state_dict.detach().to(
                "cpu", dtype=dtype
            )
        else:
            for key, value in state_dict.items():
                converted_state_dict[f"{module_name}.{key}"] = value.detach().to(
                    "cpu", dtype=dtype
                )
...
def load_ip_adapter_model(
    ...
    if path_to_file.endswith(".safetensors"):
        ...
        for combo_key, value in raw_state_dict.items():
            ...
            combined_state_dict[module_name][".".join(key_split)] = value.detach().to(
                device, dtype=dtype
            )
