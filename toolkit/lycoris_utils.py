...
def extract_conv(
    ...
    if mode == "fixed":
        ...
        assert mode_param >= 0
        lora_rank = torch.sum(mode_param < S)
    elif mode == "ratio":
        assert 1 >= mode_param >= 0
        min_s = torch.max(S) * mode_param
        lora_rank = torch.sum(min_s < S)
...
def merge(base_model, lyco_state_dict, scale: float = 1.0, device="cpu"):
    UNET_TARGET_REPLACE_MODULE = [
        "Transformer2DModel",
        "Attention",
        "ResnetBlock2D",
    ...
    def merge_state_dict(
        ...
        for name, module in tqdm(
            ...
            if module.__class__.__name__ in target_replace_modules:
                ...
                result = rebuild_weight(
                    *get_module(lyco_state_dict, lora_name), module.weight, scale
                )
                if result is not None:
                    merged += 1
