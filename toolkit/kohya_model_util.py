...
def convert_ldm_unet_checkpoint(v2, checkpoint, config):
    ...
    for i in range(1, num_input_blocks):
        ...
        if f"input_blocks.{i}.0.op.weight" in unet_state_dict:
            ...
            mapping[f"input_blocks.{i}.0.op.weight"] = (
            ...
            mapping[f"input_blocks.{i}.0.op.bias"] = (
...
def convert_ldm_vae_checkpoint(checkpoint, config):
    ...
    for i in range(num_down_blocks):
        ...
        if f"encoder.down.{i}.downsample.conv.weight" in vae_state_dict:
            ...
            mapping[f"encoder.down.{i}.downsample.conv.weight"] = (
                f"encoder.down_blocks.{i}.downsamplers.0.conv.weight"
            )
            new_checkpoint[f"encoder.down_blocks.{i}.downsamplers.0.conv.bias"] = (
                vae_state_dict.pop(f"encoder.down.{i}.downsample.conv.bias")
            )
            mapping[f"encoder.down.{i}.downsample.conv.bias"] = (
    ...
    for i in range(num_up_blocks):
        ...
        if f"decoder.up.{block_id}.upsample.conv.weight" in vae_state_dict:
            ...
            mapping[f"decoder.up.{block_id}.upsample.conv.weight"] = (
                f"decoder.up_blocks.{i}.upsamplers.0.conv.weight"
            )
            new_checkpoint[f"decoder.up_blocks.{i}.upsamplers.0.conv.bias"] = (
                vae_state_dict[f"decoder.up.{block_id}.upsample.conv.bias"]
            )
            mapping[f"decoder.up.{block_id}.upsample.conv.bias"] = (
...
def convert_ldm_clip_checkpoint_v1(checkpoint):
    ...
    if "text_model.embeddings.position_ids" not in text_model_dict:
        text_model_dict["text_model.embeddings.position_ids"] = torch.arange(
            77
        ).unsqueeze(0)  # 77 is the max length of the text
...
def load_models_from_stable_diffusion_checkpoint(
    v2, ckpt_path, device="cpu", dtype=None, unet_use_linear_projection_in_v2=False
):
