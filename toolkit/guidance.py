...
def get_differential_mask(
    ...
    if len(differential_mask.shape) == 4:
        max_differential = (
            differential_mask.max(dim=1, keepdim=True)[0]
...
def get_targeted_polarity_loss(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        unconditional_latents = batch.unconditional_latents.to(
            device, dtype=dtype
        ).detach()
        ...
        unconditional_diff_noise = unconditional_diff_noise.detach().requires_grad_(
            False
        )
        #
        baseline_conditional_noisy_latents = sd.add_noise(
            conditional_latents, noise, timesteps
        ).detach()

        baseline_unconditional_noisy_latents = sd.add_noise(
            unconditional_latents, noise, timesteps
        ).detach()
        ...
        conditional_noisy_latents = sd.add_noise(
            conditional_latents, conditional_noise, timesteps
        ).detach()

        unconditional_noisy_latents = sd.add_noise(
            unconditional_latents, unconditional_noise, timesteps
        ).detach()
        ...
        cat_latents = torch.cat(
            [conditional_noisy_latents, unconditional_noisy_latents], dim=0
        )
    ...
    pred_loss = torch.nn.functional.mse_loss(
        pred_pos.float(), conditional_noise.float(), reduction="none"
    )
    pred_loss = pred_loss.mean([1, 2, 3])

    pred_neg_loss = torch.nn.functional.mse_loss(
        pred_neg.float(), unconditional_noise.float(), reduction="none"
    )
...
def get_direct_guidance_loss(
    ...
    **kwargs,
    ...
    prediction = sd.predict_noise(
        latents=torch.cat([unconditional_noisy_latents, conditional_noisy_latents])
        .to(device, dtype=dtype)
        .detach(),
        conditional_embeddings=concat_prompt_embeds(
    ...
    guidance_loss = torch.nn.functional.mse_loss(
        guidance_pred.float(), noise.detach().float(), reduction="none"
    )
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        target_differential_abs_min = (
            target_differential_abs.min(dim=1, keepdim=True)[0]
            .max(dim=2, keepdim=True)[0]
            .max(dim=3, keepdim=True)[0]
        )
        target_differential_abs_max = (
            target_differential_abs.max(dim=1, keepdim=True)[0]
            .max(dim=2, keepdim=True)[0]
            .max(dim=3, keepdim=True)[0]
        )
        ...
        prior_prediction_loss = (
            torch.nn.functional.mse_loss(
                target_unconditional.float(), noise.float(), reduction="none"
            )
            .detach()
            .clone()
        )
    ...
    sd.network.is_active = True
    sd.network.multiplier = network_weight_list + [
        x + -1.0 for x in network_weight_list
    ]
    ...
    conditional_loss = torch.nn.functional.mse_loss(
        prediction_conditional.float(), noise.float(), reduction="none"
    )

    unconditional_loss = torch.nn.functional.mse_loss(
        prediction_unconditional.float(), noise.float(), reduction="none"
    )
...
def get_guided_loss_polarity(
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        conditional_noisy_latents = sd.condition_noisy_latents(
            conditional_noisy_latents, batch
        )

        unconditional_noisy_latents = sd.add_noise(
            unconditional_latents, noise, timesteps
        ).detach()
        unconditional_noisy_latents = sd.condition_noisy_latents(
            unconditional_noisy_latents, batch
        )
    ...
    pred_loss = torch.nn.functional.mse_loss(
        pred_pos.float(), target_pos.float(), reduction="none"
    )
    # pred_loss = pred_loss.mean([1, 2, 3])

    pred_neg_loss = torch.nn.functional.mse_loss(
        pred_neg.float(), target_neg.float(), reduction="none"
    )
...
def get_guided_tnt(
    ...
    **kwargs,
    ...
    this_loss = torch.nn.functional.mse_loss(
        this_prediction.float(), noise.float(), reduction="none"
    )

    that_loss = torch.nn.functional.mse_loss(
        that_prediction.float(), noise.float(), reduction="none"
    )
    ...
    **kwargs,
    ...
    with torch.no_grad():
        ...
        abs_differential_mask = get_differential_mask(
            conditional_latents, unconditional_latents, gradient=True
        )
        ...
        baseline_predicted_noise = (
            1 - abs_differential_mask
        ) * baseline_predicted_noise
    ...
    pred_loss = torch.nn.functional.mse_loss(prediction.float(), target_pred.float())
...
def get_guidance_loss(
    ...
    **kwargs,
    ...
    if guidance_type == "targeted":
        assert unconditional_embeds is None, (
            "Unconditional embeds are not supported for targeted guidance"
        )
        ...
        assert unconditional_embeds is None, (
            "Unconditional embeds are not supported for polarity guidance"
        )
        ...
        assert unconditional_embeds is None, (
            "Unconditional embeds are not supported for targeted polarity guidance"
        )
