            ...
            if self.adapter_ref().reference_mode == "write":
                ...
                if self._memory is None:
                    ...
                    try:
                        ...
                        hidden_states = (
                            blend * new_hidden_states + (1 - blend) * hidden_states
                        )
...
class ReferenceAdapter(torch.nn.Module):
    def __init__(self, sd: "StableDiffusion", adapter_config: "AdapterConfig"):
        ...
        self.noise_scheduler: DDPMScheduler | EulerDiscreteScheduler = None
    ...
    def load_reference_adapter(self, state_dict: OrderedDict | dict):
    ...
    def set_reference_images(self, reference_images: torch.Tensor | None):
        self._reference_images = reference_images.clone().detach()
        self._reference_latents = None
        self.clear_memory()

    def set_blank_reference_images(self, batch_size):
        self._reference_images = torch.zeros(
            (batch_size, 3, 512, 512),
            device=self.device,
            dtype=self.sd_ref().torch_dtype,
        )
        self._reference_latents = torch.zeros(
            (batch_size, 4, 64, 64), device=self.device, dtype=self.sd_ref().torch_dtype
        )
        self.clear_memory()
    ...
    def unet_forward(self, sample, timestep, encoder_hidden_states, *args, **kwargs):
        ...
        if not skip:
            ...
            if self.sd_ref().network.is_merged_in:
                raise ValueError(
                    "network is merged in, but we are not supposed to be merged in"
                )
    ...
    def forward(self, sample, timestep, encoder_hidden_states, *args, **kwargs):
        ...
        if not self.is_active or (
            self._reference_images is None and self._reference_latents is None
        ):
        ...
        if self._reference_latents is None:
            self._reference_latents = (
                self.sd_ref()
                .encode_images(
                    self._reference_images.to(self.device, self.sd_ref().torch_dtype)
                )
                .detach()
            )
        # create a sample from our reference images
        reference_latents = (
            self._reference_latents.clone()
            .detach()
            .to(self.device, self.sd_ref().torch_dtype)
        )
        ...
        if reference_latents.shape[0] * 2 == sample.shape[0]:
            ...
            reference_latents = torch.cat(
                [torch.zeros_like(reference_latents), reference_latents], dim=0
            ).detach()
        ...
        reference_latents = F.interpolate(
            reference_latents,
            size=(
                int(reference_latents.shape[2] * scale),
                int(reference_latents.shape[3] * scale),
            ),
            mode=mode,
            align_corners=False,
        )
        ...
        reference_latents = F.pad(
            reference_latents,
            (
                math.floor(width_pad),
                math.floor(width_pad),
                math.ceil(height_pad),
                math.ceil(height_pad),
            ),
            mode="constant",
            value=0,
        ...
        self._original_unet_forward(
            reference_latents, timestep, encoder_hidden_states, *args, **kwargs
        )
        self.reference_mode = "read"
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: bool = True):
        ...
        self.adapter_modules.load_state_dict(
            state_dict["reference_adapter"], strict=strict
        )
