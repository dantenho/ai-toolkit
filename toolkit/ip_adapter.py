...
import torch
from diffusers import FluxTransformer2DModel, Transformer2DModel
...
from toolkit.util.ip_adapter_utils import (
    AttnProcessor2_0,
    ImageProjModel,
    IPAttnProcessor2_0,
)
...
class CustomIPAttentionProcessor(IPAttnProcessor2_0):
    def __init__(
        ...
        super().__init__(
            hidden_size, cross_attention_dim, scale=scale, num_tokens=num_tokens
        )
        self.adapter_ref: weakref.ref = weakref.ref(adapter)
        self.train_scaler = train_scaler
        if train_scaler:
            if full_token_scaler:
                self.ip_scaler = torch.nn.Parameter(
                    torch.ones([num_tokens], dtype=torch.float32) * 0.999
                )
            else:
                self.ip_scaler = torch.nn.Parameter(
                    torch.ones([1], dtype=torch.float32) * 0.999
                )
        ...
        if not is_active:
            ...
            if ip_hidden_states.shape[0] * 2 == batch_size:
                ...
                ip_hidden_states = torch.cat(
                    [self.adapter_ref().last_unconditional, ip_hidden_states], dim=0
                )
...
class IPAdapter(torch.nn.Module):
    ...
    def __init__(self, sd: "StableDiffusion", adapter_config: "AdapterConfig"):
        ...
        self.preprocessor: CLIPImagePreProcessor | None = None
        self.input_size = 224
        ...
        if self.config.image_encoder_arch.startswith("clip"):
            ...
            try:
                self.clip_image_processor = SiglipImageProcessor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except OSError:
            ...
            try:
                self.clip_image_processor = ViTFeatureExtractor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except OSError:
                self.clip_image_processor = ViTFeatureExtractor()
            self.image_encoder = ViTForImageClassification.from_pretrained(
                adapter_config.image_encoder_path
            ).to(self.device, dtype=get_torch_dtype(self.sd_ref().dtype))
        elif self.config.image_encoder_arch == "safe":
            try:
                self.clip_image_processor = SAFEImageProcessor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except OSError:
            ...
            try:
                self.clip_image_processor = ConvNextImageProcessor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except OSError:
                print(
            ...
            try:
                self.clip_image_processor = AutoImageProcessor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except OSError:
                print(
            ...
            try:
                self.clip_image_processor = ViTHybridImageProcessor.from_pretrained(
                    adapter_config.image_encoder_path
                )
            except OSError:
                print(
        ...
        if not self.config.image_encoder_arch == "safe":
            if "height" in self.clip_image_processor.size:
                ...
                raise ValueError(
                    f"unknown image processor size: {self.clip_image_processor.size}"
                )
        ...
        if adapter_config.type == "ip":
            ...
            cross_attn_dim = (
                4096 if is_pixart else sd.unet.config["cross_attention_dim"]
            )
            ...
            embedding_dim = (
                ...
                if not self.config.image_encoder_arch.startswith("convnext")
                else self.image_encoder.config.hidden_sizes[-1]
            )
            ...
            out_tokens = (
                self.config.num_tokens if self.config.num_tokens > 0 else in_tokens
            )
            # ip-adapter-plus
        ...
        for name in attn_processor_keys:
            ...
            transformer_idx = (
                name_split.index("transformer_blocks")
                if "transformer_blocks" in name_split
                else -1
            )
            if transformer_idx >= 0:
                transformer_name = ".".join(name_split[:2])
                transformer_name += "." + ".".join(
                    name_split[transformer_idx : transformer_idx + 2]
                )
            ...
            if is_flux:
                ...
                cross_attention_dim = (
                    ...
                    else sd.unet.config["cross_attention_dim"]
                )
            if name.startswith("mid_block"):
                hidden_size = sd.unet.config["block_out_channels"][-1]
            elif name.startswith("up_blocks"):
                block_id = int(name[len("up_blocks.")])
                hidden_size = list(reversed(sd.unet.config["block_out_channels"]))[
                    block_id
                ]
            elif name.startswith("down_blocks"):
                block_id = int(name[len("down_blocks.")])
                hidden_size = sd.unet.config["block_out_channels"][block_id]
            elif name.startswith("transformer") or name.startswith(
                "single_transformer"
            ):
                if is_flux:
                    hidden_size = 3072
            ...
            if cross_attention_dim is None and not is_flux:
                ...
                weights = {"to_k_ip.weight": k_weight, "to_v_ip.weight": v_weight}
        ...
        if self.sd_ref().is_pixart:
            ...
            self.adapter_modules = torch.nn.ModuleList(
                [
                    transformer.transformer_blocks[i].attn2.processor
            ...
            for i, module in transformer.single_transformer_blocks.named_children():
                module.attn.processor = attn_procs[
                    f"single_transformer_blocks.{i}.attn"
                ]

            self.adapter_modules = torch.nn.ModuleList(
                [
                    transformer.transformer_blocks[i].attn.processor
                    for i in range(len(transformer.transformer_blocks))
                ]
                + [
                    transformer.single_transformer_blocks[i].attn.processor
        ...
        if self.config.name_or_path:
            loaded_state_dict = load_ip_adapter_model(
                self.config.name_or_path, device="cpu", dtype=sd.torch_dtype
            )
                ...
                if key.endswith("ip_scaler"):
                    ...
                    for key2 in list(state_dict["ip_adapter"].keys()):
                        if key2.endswith(f"{key_start}.to_k_ip.weight"):
                            state_dict["ip_adapter"][key2] = (
                                state_dict["ip_adapter"][key2].clone() * scale
                            )
    ...
    def parse_clip_image_embeds_from_cache(
        ...
        image_embeds_list: list[
            dict
        ],  # has ['last_hidden_state', 'image_embeds', 'penultimate_hidden_states']
        quad_count=4,
    ):
        with torch.no_grad():
            device = self.sd_ref().unet.device
            clip_image_embeds = torch.cat(
                [x[self.config.clip_layer] for x in image_embeds_list], dim=0
            )
            ...
            clip_image_embeds = clip_image_embeds.to(
                device, dtype=get_torch_dtype(self.sd_ref().dtype)
            ).detach()
        return clip_image_embeds

    def get_empty_clip_image(self, batch_size: int) -> torch.Tensor:
        with torch.no_grad():
            tensors_0_1 = torch.rand(
                [batch_size, 3, self.input_size, self.input_size], device=self.device
            )
        ...
        with torch.no_grad():
            ...
            if not has_been_preprocessed:
                ...
                if tensors_0_1.min() < -0.3 or tensors_0_1.max() > 1.3:
                    raise ValueError(
                        f"image tensor values must be between 0 and 1. Got min: {tensors_0_1.min()}, max: {tensors_0_1.max()}"
                    )
                # unconditional
                if drop:
                ...
                if drop:
                    ...
                    tensors_0_1 = (
                        torch.clip((255.0 * tensors_0_1), 0, 255).round() / 255.0
                    )
                    clip_image = (tensors_0_1 - mean.view([1, 3, 1, 1])) / std.view(
                        [1, 3, 1, 1]
                    )
        ...
        with torch.set_grad_enabled(is_training):
            ...
            if self.config.image_encoder_arch.startswith("convnext"):
                ...
                clip_image_embeds = clip_image_embeds.view(
                    clip_image_embeds.size(0), clip_image_embeds.size(1), -1
                )
            ...
            with torch.no_grad():
                if cfg_embed_strength is not None:
                    uncond_clip = self.get_empty_clip_image(tensors_0_1.shape[0]).to(
                        self.device, dtype=get_torch_dtype(self.sd_ref().dtype)
                    )
                    if self.config.quad_image:
                        # split the 4x4 grid and stack on batch
                    ...
                    if self.config.clip_layer == "penultimate_hidden_states":
                        ...
                        if self.config.adapter_type == "clip_face":
                            ...
                            uncond_clip_output_embeds = (
                                uncond_clip_output_embeds / l2_norm
                            )
                    ...
                    clip_image_embeds = inverse_classifier_guidance(
                        clip_image_embeds, uncond_clip_output_embeds, cfg_embed_strength
                    )
    ...
    def forward(
        ...
        if self.sd_ref().is_flux:
            ...
            embeddings.text_embeds = torch.cat(
                [embeddings.text_embeds, image_prompt_embeds], dim=1
            )
        return embeddings
    ...
    def get_parameter_groups(self, adapter_lr):
        ...
        if self.config.train_scaler or self.config.merge_scaler:
            scaler_lr = (
                adapter_lr if self.config.scaler_lr is None else self.config.scaler_lr
            )
            param_groups.append(
                {
    ...
    def get_non_scaler_parameters(self, recurse: bool = True) -> Iterator[Parameter]:
        if self.config.train_only_image_encoder:
            if self.config.train_only_image_encoder_positional_embedding:
                yield from self.image_encoder.vision_model.embeddings.position_embedding.parameters(
                    recurse
                )
            else:
                ...
                if current_shape != new_shape:
                    try:
                        ...
                        if len(current_shape) == 1:
                            ...
                            current_img_proj_state_dict[key][
                        ...
                        if len(current_shape) == 1:
                            current_img_proj_state_dict[key][: current_shape[0]] = (
                                value[: current_shape[0]]
                            )
                        elif len(current_shape) == 2:
                            current_img_proj_state_dict[key][
                                : current_shape[0], : current_shape[1]
                            ] = value[: current_shape[0], : current_shape[1]]
                        elif len(current_shape) == 3:
                            current_img_proj_state_dict[key][
                                : current_shape[0],
                                : current_shape[1],
                            ...
                            raise ValueError(f"unknown shape: {current_shape}")
                        print(
                            f"Force merged in {key}: {list(current_shape)} <<< {list(new_shape)}"
                        )
                else:
                ...
                if current_shape != new_shape:
                    try:
                        ...
                        if len(current_shape) == 1:
                            ...
                            current_ip_adapter_state_dict[key][
                                : new_shape[0], : new_shape[1]
                            ] = value
                        elif len(current_shape) == 3:
                            current_ip_adapter_state_dict[key][
                        ...
                        if len(current_shape) == 1:
                            current_ip_adapter_state_dict[key][: current_shape[0]] = (
                                value[: current_shape[0]]
                            )
                        elif len(current_shape) == 2:
                            current_ip_adapter_state_dict[key][
    ...
    def load_state_dict(self, state_dict: Mapping[str, Any], strict: bool = True):
        ...
        if "ip_adapter" in state_dict:
            try:
                self.image_proj_model.load_state_dict(
                    state_dict["image_proj"], strict=strict
                )
                self.adapter_modules.load_state_dict(
                    state_dict["ip_adapter"], strict=strict
                )
            except Exception as e:
                print(e)
        ...
        if self.config.train_image_encoder and "image_encoder" in state_dict:
            self.image_encoder.load_state_dict(
                state_dict["image_encoder"], strict=strict
            )
        if self.preprocessor is not None and "preprocessor" in state_dict:
