...
class ModelCache:
    ...
    def get_model_path(self, model_id: int, model_version_id: int = None):
        ...
        if model_version_id is None:
            # get latest version
            model_version_id = max(
                [int(x) for x in self.raw_cache[str(model_id)].keys()]
            )
            if model_version_id is None:
                return None
            model_path = self.raw_cache[str(model_id)][str(model_version_id)][
                "model_path"
            ]
...
def get_model_download_info(model_id: int, model_version_id: int = None):
    ...
    if model_file is None:
        raise ValueError(
            f"Could not find a model file to download for model id: {model_id}"
        )
...
def get_model_path_from_url(url: str):
    ...
    model_path = model_cache.get_model_path(
        model_id, query_params.get("modelVersionId")
    )
    if model_path is not None:
        return model_path
    else:
        # download model
        file_info, model_version_id = get_model_download_info(
            model_id, query_params.get("modelVersionId")
        )
        ...
        # download model
        print(
            f"Did not find model locally, downloading from model from: {download_url}"
        )
...
if __name__ == "__main__":
    model_path = get_model_path_from_url(
        "https://civitai.com/models/25694?modelVersionId=127742"
    )
    print(model_path)
