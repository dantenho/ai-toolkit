...
ExtractMode = Union["fixed", "threshold", "ratio", "quantile", "percentage"]
...
def extract_conv(
    weight: torch.Tensor | nn.Parameter, mode="fixed", mode_param=0, device="cpu"
    ...
    if mode == "percentage":
        ...
        assert mode_param >= 0
        lora_rank = torch.sum(mode_param < S).item()
    elif mode == "ratio":
        assert 1 >= mode_param >= 0
        min_s = torch.max(S) * mode_param
        lora_rank = torch.sum(min_s < S).item()
    ...
    if lora_rank >= out_ch / 2:
        ...
        print("rank is higher than it should be")
...
def convert_diffusers_unet_to_lorm(
    ...
    for name, module in named_modules:
        ...
        if module_name in UNET_TARGET_REPLACE_MODULE:
            for child_name, child_module in module.named_modules():
                new_module: LoRMCon2d | LoRMLinear | None = None
                ...
                if any([word in child_name for word in ignore_if_contains]):
                    ...
                    if count_parameters(child_module) > parameter_threshold:
                        ...
                        if child_module.bias is not None:
                            bias_weight = (
                                child_module.bias.data.clone().detach().to(dtype=dtype)
                            )
    ...
    sorted_layer_names_replaced = sorted(
        layer_names_replaced, key=sorting_key, reverse=True
    )
