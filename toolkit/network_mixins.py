...
from toolkit.lorm import count_parameters, extract_conv, extract_linear
...
if TYPE_CHECKING:
    ...
    from toolkit.lycoris_special import LoConSpecialModule, LycorisSpecialNetwork
...
ExtractMode = Union["existingfixed", "threshold", "ratio", "quantile", "percentage"]
...
class ExtractableModuleMixin:
    def extract_weight(
        ...
        extract_mode_param: int | float = None,
    ):
        ...
        if self.org_module[0].__class__.__name__ in CONV_MODULES:
            ...
            raise ValueError(
                f"Unknown module type: {self.org_module[0].__class__.__name__}"
            )
        ...
        self.lora_down.weight.data = (
            down_weight.to(self.lora_down.weight.dtype).clone().detach()
        )
        self.lora_up.weight.data = (
            up_weight.to(self.lora_up.weight.dtype).clone().detach()
        )
        ...
        if hasattr(self, "scalar"):
            ...
            self.scalar.data = torch.tensor(1.0).to(
                self.scalar.device, self.scalar.dtype
            )


class ToolkitModuleMixin:
    def __init__(self: Module, *args, network: Network, **kwargs):
        self.network_ref: weakref.ref = weakref.ref(network)
        self.is_checkpointing = False
        self._multiplier: float | list | torch.Tensor = None
    ...
    def _call_forward(self: Module, x):
        ...
        if isinstance(self.dropout, nn.Dropout) or isinstance(
        ...
        if network.lorm_train_mode == "local":
            ...
            with torch.set_grad_enabled(True):
                ...
                local_loss = torch.nn.functional.mse_loss(
                    target_pred.float(), lorm_pred.float()
                )
                # backpropr
                local_loss.backward()
    ...
    def setup_lorm(self: Module, state_dict: dict[str, Any] | None = None):
        ...
        network: Network = self.network_ref()
        lorm_config = network.network_config.lorm_config.get_config_for_module(
            self.lora_name
        )
        ...
        self.extract_weight(
            extract_mode=extract_mode, extract_mode_param=extract_mode_param
        )
...
class ToolkitNetworkMixin:
    def __init__(
        self: Network,
        *args,
        train_text_encoder: bool | None = True,
        train_unet: bool | None = True,
        is_sdxl=False,
        is_v2=False,
        is_ssd=False,
        is_vega=False,
    ...
    def get_state_dict(self: Network, extra_state_dict=None, dtype=torch.float16):
        ...
        if self.base_model_ref is not None:
            save_dict = self.base_model_ref().convert_lora_weights_before_save(
                save_dict
            )
        return save_dict
    ...
    def save_weights(
        ...
        dtype=torch.float16,
        metadata=None,
        extra_state_dict: OrderedDict | None = None,
    ):
        ...
        if self.base_model_ref is not None and hasattr(
            self.base_model_ref(), "save_lora"
        ):
    ...
    def load_weights(self: Network, file, force_weight_mapping=False):
        ...
        if self.base_model_ref is not None:
            weights_sd = self.base_model_ref().convert_lora_weights_before_load(
                weights_sd
            )
        ...
        for key in list(load_sd.keys()):
            if key not in current_state_dict:
                ...
                if len(load_sd[key].shape) == 2:
                    ...
                    blank_val = current_state_dict[
                        key
                    ]  # shape we need in the target model
                    ...
                    if (src_h, src_w) == (tgt_h, tgt_w):
                        ...
                        print_once(
                            f"Expanding {key} from {load_value.shape} to {blank_val.shape}"
                        )
                        new_val = torch.zeros(
                            (tgt_h, tgt_w),
                            device=load_value.device,
                            dtype=load_value.dtype,
                        )
                        new_val[:src_h, :src_w] = (
                            load_value  # src_w should already match
                        )
                        load_sd[key] = new_val
                        ...
                        new_val[:src_h, :src_w] = (
                            load_value  # src_h should already match
                        )
                        load_sd[key] = new_val
                        self.did_change_weights = True
                        ...
                        print_once(
                            f"Shrinking {key} from {load_value.shape} to {blank_val.shape}"
                        )
                        ...
                        raise ValueError(
                            f"Unhandled LoRA shape change for {key}: src={load_value.shape}, tgt={blank_val.shape}"
                        )
        ...
        if (
            len(to_delete) > 0
            and self.is_v1
            and not force_weight_mapping
            and not (len(to_delete) == 1 and "emb_params" in to_delete)
        ):
    ...
    def _update_torch_multiplier(self: Network):
        ...
        try:
            ...
            raise ValueError(
                "There are not any lora modules in this network. Check your config and try again"
            )
    ...
    @property
    def multiplier(self) -> float | list[float] | list[list[float]]:
        return self._multiplier

    @multiplier.setter
    def multiplier(self, value: float | list[float] | list[list[float]]):
    ...
    def setup_lorm(self: Network, state_dict: dict[str, Any] | None = None):
    ...
    def calculate_lorem_parameter_reduction(self):
        ...
        for module in self.get_all_modules():
            ...
            num_lorem_params = count_parameters(module.lora_down) + count_parameters(
                module.lora_up
            )
