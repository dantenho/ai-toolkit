...
class ImageDataset(Dataset, CaptionMixin):
    def __init__(self, config):
        ...
        print_acc("  -  Preprocessing image dimensions")
        new_file_list = []
        bad_count = 0
...
class AiToolkitDataset(
    ...
    def __init__(
        ...
        if self.sd is None and self.is_caching_latents:
            raise ValueError("sd is required for caching latents")
        ...
        if self.is_video:
            print_acc("  -  Preprocessing video dimensions")
        else:
            print_acc("  -  Preprocessing image dimensions")
        ...
        if os.path.exists(dataset_size_file):
            try:
                with open(dataset_size_file) as f:
        ...
        for file in tqdm(file_list):
            try:
                file_item = FileItemDTO(
                    ...
                    encode_control_in_text_embeddings=self.sd.encode_control_in_text_embeddings
                    if self.sd
                    else False,
                )
...
def get_dataloader_from_datasets(
    ...
    if has_buckets:
        ...
        for dataset in datasets:
            assert dataset.dataset_config.buckets, (
