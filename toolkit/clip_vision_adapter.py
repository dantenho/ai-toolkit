...
class ClipVisionAdapter(torch.nn.Module):
    def __init__(self, sd: "StableDiffusion", adapter_config: AdapterConfig):
        ...
        self.text_encoder_list = (
            sd.text_encoder if isinstance(sd.text_encoder, list) else [sd.text_encoder]
        )
        self.tokenizer_list = (
            sd.tokenizer if isinstance(sd.tokenizer, list) else [sd.tokenizer]
        )
        ...
        self.tokenizer_list = (
            ...
            else [self.sd_ref().tokenizer]
        )
        self.text_encoder_list = (
            self.sd_ref().text_encoder
            if isinstance(self.sd_ref().text_encoder, list)
            else [self.sd_ref().text_encoder]
        )
        ...
        for text_encoder, tokenizer in zip(self.text_encoder_list, self.tokenizer_list):
            ...
            init_token_ids = tokenizer.encode(
                self.config.trigger_class_name, add_special_tokens=False
            )
            ...
            if len(init_token_ids) > self.config.num_tokens:
                ...
                init_token_ids += pad_token_id * (
                    self.config.num_tokens - len(init_token_ids)
                )
        ...
        self.image_encoder = CLIPVisionModelWithProjection.from_pretrained(
            self.config.image_encoder_path, ignore_mismatched_sizes=True
    ...
    def set_vec(self, new_vector, text_encoder_idx=0):
        ...
        embedding_layer = self.text_encoder_list[
            text_encoder_idx
    ...
    def forward(self, clip_image_embeds: torch.Tensor) -> PromptEmbeds:
        ...
        if len(self.text_encoder_list) == 1:
            ...
            if (
                self.text_encoder_list[0].config.target_hidden_size
                + self.text_encoder_list[1].config.target_hidden_size
                != image_prompt_embeds.shape[2]
            ):
                raise ValueError(
                    "Something went wrong. The embeddings do not match the text encoder sizes"
                )
            # sdxl variants
            # image_prompt_embeds = 2048
            # te1 = 768
            # te2 = 1280
            te1_embeds = image_prompt_embeds[
                :, :, : self.text_encoder_list[0].config.target_hidden_size
            ]
            te2_embeds = image_prompt_embeds[
                :, :, self.text_encoder_list[0].config.target_hidden_size :
            ]
